...
const inputCls = "w-full bg-[#100C18] border border-white/[0.08] rounded-xl px-3.5 py-2.5 text-[13px] text-[#E8E9EC] placeholder:text-[#4B5563] focus:outline-none focus:border-[#6B21A8]/50 transition-colors";
...
function StatCard({ label, value, positive, negative, icon: Icon, sub, compact }) {
  ...
  return (
    <div className={`rounded-2xl bg-[#0B0810] border border-white/[0.06] overflow-hidden min-w-0 ${compact ? 'p-3' : 'p-4'}`}>
      <div className="flex items-center gap-1.5 text-[#6B7280] mb-2 min-w-0">{Icon && <Icon size={13} className="shrink-0" />}<span className="text-[9px] font-medium uppercase tracking-wide truncate">{label}</span></div>
      <p className={`font-display font-semibold tracking-tight truncate ${compact ? 'text-[13px]' : 'text-[17px]'}`} style={{ color }}>{value}</p>
...
function DashboardView({ trades, loading }) {
  ...
  return (
    <>
      {loading ? <div className="text-center py-16 text-[#6B7280] text-[13px]">Loading from database...</div> : !hasTrades ? (
        <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-6 text-center">
          <p className="text-[13px] font-medium mb-1">No trades yet</p>
          <p className="text-[13px] text-[#6B7280] leading-relaxed">Log a trade on the Trades tab and this screen fills in automatically.</p>
        </div>
      ) : null}
      <div className="rounded-2xl bg-gradient-to-br from-[#0B0810] to-[#08060D] border border-white/[0.06] p-5">
        <p className="text-[10px] uppercase tracking-[0.14em] text-[#6B7280] mb-1">Total P&L</p>
        <p className="font-display text-[24px] font-semibold tracking-tight" style={{ color: totalPnl > 0 ? '#22C55E' : totalPnl < 0 ? '#EF4444' : '#E8E9EC' }}>{fmtMoney(totalPnl)}</p>
...
function TradeForm({ initial, onSave, saveLabel }) {
  ...
  return (
    <>
      <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-5 space-y-4">
        <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="Direction">
          <div className="grid grid-cols-2 gap-2">
            {['long', 'short'].map(d => (
              <button key={d} onClick={() => setForm(f => ({ ...f, direction: d }))} className={`py-2.5 rounded-xl text-[13px] font-medium border transition-colors ${form.direction === d ? (d === 'long' ? 'bg-[#22C55E]/15 border-[#22C55E]/50 text-[#22C55E]' : 'bg-[#EF4444]/15 border-[#EF4444]/50 text-[#EF4444]') : 'bg-[#100C18] border-white/[0.08] text-[#6B7280]'}`}>{d === 'long' ? 'Long' : 'Short'}</button>
            ))}
          </div>
        </Field>
      </div>
      <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-5 space-y-4">
        ...
        {hasNumbers && (
          <div className="grid grid-cols-3 gap-2 pt-1">
            <div className="rounded-xl bg-[#100C18] border border-white/[0.06] p-3 text-center"><p className="text-[9px] uppercase text-[#6B7280] mb-1">Risk</p><p className="text-[13px] font-semibold">${riskDollar.toFixed(2)}</p></div>
            <div className="rounded-xl bg-[#100C18] border border-white/[0.06] p-3 text-center"><p className="text-[9px] uppercase text-[#6B7280] mb-1">P&L</p><p className="text-[13px] font-semibold" style={{ color: pnl > 0 ? '#22C55E' : pnl < 0 ? '#EF4444' : '#E8E9EC' }}>{pnl > 0 ? '+' : ''}{pnl.toFixed(2)}</p></div>
            <div className="rounded-xl bg-[#100C18] border border-white/[0.06] p-3 text-center"><p className="text-[9px] uppercase text-[#6B7280] mb-1">R Multiple</p><p className="text-[13px] font-semibold" style={{ color: rMultiple > 0 ? '#22C55E' : rMultiple < 0 ? '#EF4444' : '#E8E9EC' }}>{rMultiple.toFixed(2)}R</p></div>
          </div>
        )}
      </div>
      <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-5 space-y-4">
        ...
        <Field label="Screenshot">
          {form.screenshotUrl ? (
            <div className="relative">
              ...
              <p className="text-[9px] text-[#6B7280] mt-1.5 text-center">Tap image to view full size</p>
            </div>
          ) : (
            <label className="w-full bg-[#100C18] border border-dashed border-white/[0.15] rounded-xl px-3.5 py-5 text-center block cursor-pointer">
      ...
      {zoomedImage && form.screenshotUrl && (
        <div onClick={() => setZoomedImage(false)} className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4" style={{ position: 'fixed' }}>
          ...
          <button onClick={() => setZoomedImage(false)} className="absolute top-6 right-6 bg-white/10 text-white w-9 h-9 rounded-full flex items-center justify-center text-[15px]">✕</button>
        </div>
      )}
      <button onClick={handleSave} disabled={!canSave || saving} className={`w-full py-3.5 rounded-xl font-display text-[13px] font-semibold transition-colors flex items-center justify-center gap-2 ${canSave ? 'bg-[#6B21A8] text-black' : 'bg-[#100C18] text-[#4B5563] border border-white/[0.06]'}`}>
  ...
  return (
    <>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 -mx-0.5 px-0.5">
        {FILTERS.map(f => (<button key={f} onClick={() => setFilter(f)} className={`shrink-0 px-3 py-1.5 rounded-lg text-[11px] font-medium border transition-colors ${filter === f ? 'bg-[#6B21A8]/15 border-[#6B21A8]/50 text-[#6B21A8]' : 'bg-[#100C18] border-white/[0.08] text-[#6B7280]'}`}>{f}</button>))}
      </div>
      <select value={sort} onChange={e => setSort(e.target.value)} className={inputCls}>{SORTS.map(s => <option key={s} value={s}>{s}</option>)}</select>
      {list.length === 0 ? (
        <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-6 text-center"><p className="text-[13px] text-[#6B7280]">No trades match.</p></div>
      ) : (
        <div className="space-y-2">
          {list.map(t => (
            <button key={t.id} onClick={() => onEdit(t)} className="w-full text-left rounded-xl bg-[#0B0810] border border-white/[0.06] px-4 py-3 flex items-center justify-between active:bg-[#100C18] transition-colors">
              <div><p className="font-display text-[13px] font-semibold flex items-center gap-1.5">{t.symbol} <span className="text-[#6B7280] font-normal">· {t.direction === 'long' ? 'Long' : 'Short'} · {t.market}</span>{t.screenshotUrl && <Camera size={12} className="text-[#6B7280] shrink-0" />}</p><p className="text-[10px] text-[#6B7280]">{t.date}</p></div>
              <div className="text-right"><p className="text-[13px] font-semibold" style={{ color: t.pnl > 0 ? '#22C55E' : t.pnl < 0 ? '#EF4444' : '#E8E9EC' }}>{t.pnl > 0 ? '+' : ''}{Number(t.pnl).toFixed(2)}</p><p className="text-[10px] text-[#6B7280]">{Number(t.rMultiple).toFixed(2)}R</p></div>
            </button>
          ))}
        </div>
      )}
...
function CalendarView({ trades }) {
  ...
  return (
    <>
      <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-4 grid grid-cols-3 gap-3">
        <div><p className="text-[9px] uppercase text-[#6B7280] mb-1">Month P&L</p><p className="text-[13px] font-semibold truncate" style={{ color: monthPnl > 0 ? '#22C55E' : monthPnl < 0 ? '#EF4444' : '#E8E9EC' }}>{fmtMoney(monthPnl)}</p></div>
        <div><p className="text-[9px] uppercase text-[#6B7280] mb-1">Trades</p><p className="text-[13px] font-semibold">{monthTrades.length}</p></div>
        <div><p className="text-[9px] uppercase text-[#6B7280] mb-1">Win Rate</p><p className="text-[13px] font-semibold">{monthWinRate.toFixed(0)}%</p></div>
      </div>

      <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-2.5">
        <div className="flex items-center justify-between mb-1 px-1.5">
          <button onClick={() => { setCursor(new Date(year, month - 1, 1)); setSelected(null); }} className="p-1.5 rounded-lg bg-[#100C18] text-[#9CA3AF]"><ChevronLeft size={16} /></button>
          <p className="text-[13px] font-semibold">{MONTH_NAMES[month]} {year}</p>
          <button onClick={() => { setCursor(new Date(year, month + 1, 1)); setSelected(null); }} className="p-1.5 rounded-lg bg-[#100C18] text-[#9CA3AF]"><ChevronLeft size={16} className="rotate-180" /></button>
        </div>
        ...
        <div className="grid grid-cols-7 gap-1.5 mb-2">
          {WEEKDAYS.map((w, i) => <div key={i} className="text-center text-[9px] text-[#6B7280] font-medium">{w}</div>)}
        </div>
        <div className="grid grid-cols-7 gap-1.5">
          {cells.map((d, i) => {
            ...
            const style = info ? tierStyle(info.pnl) : { bg: '#100C18', text: '#4B5563' };
        ...
        <div className="flex items-center justify-center gap-1.5 mt-4 flex-wrap">
          <span className="text-[9px] text-[#6B7280] mr-1">Loss</span>
          {['#EF4444', '#FCA5A5', '#FEE2E2'].map(c => <div key={c} className="w-3.5 h-3.5 rounded" style={{ backgroundColor: c }} />)}
          <div className="w-3.5 h-3.5 rounded bg-[#100C18] mx-1" />
          {['#DCFCE7', '#86EFAC', '#22C55E'].map(c => <div key={c} className="w-3.5 h-3.5 rounded" style={{ backgroundColor: c }} />)}
          <span className="text-[9px] text-[#6B7280] ml-1">Profit</span>
        </div>
      </div>

      {selected && (
        <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-4">
          <p className="text-[10px] uppercase tracking-wide text-[#6B7280] mb-3">{selected}</p>
          {selectedTrades.length === 0 ? (
            <p className="text-[13px] text-[#6B7280]">No trades this day.</p>
          ) : (
            <div className="space-y-2">
              {selectedTrades.map(t => (
                <div key={t.id} className="rounded-xl bg-[#100C18] border border-white/[0.06] px-4 py-3 flex items-center justify-between">
                  <div><p className="font-display text-[13px] font-semibold">{t.symbol} <span className="text-[#6B7280] font-normal">· {t.direction === 'long' ? 'Long' : 'Short'}</span></p><p className="text-[10px] text-[#6B7280]">{t.strategy || 'No strategy noted'}</p></div>
                  <p className="text-[13px] font-semibold" style={{ color: t.pnl > 0 ? '#22C55E' : t.pnl < 0 ? '#EF4444' : '#E8E9EC' }}>{t.pnl > 0 ? '+' : ''}{Number(t.pnl).toFixed(2)}</p>
                </div>
              ))}
            </div>
          )}
        ...
        {groups.map(g => (
          <div key={g.key} className="flex items-center justify-between">
            <div className="min-w-0 flex-1 mr-3">
              <p className="text-[13px] font-medium truncate">{g.key}</p>
              <p className="text-[10px] text-[#6B7280]">{g.count} trade{g.count !== 1 ? 's' : ''} · {g.count > 0 ? Math.round((g.wins / g.count) * 100) : 0}% win</p>
            </div>
            <p className="text-[13px] font-semibold shrink-0" style={{ color: g.pnl > 0 ? '#22C55E' : g.pnl < 0 ? '#EF4444' : '#E8E9EC' }}>{g.pnl > 0 ? '+' : ''}{g.pnl.toFixed(2)}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <button onClick={onClose} className="flex items-center gap-1 text-[13px] text-[#6B7280]"><ChevronLeft size={16} /> Back to Stats</button>
      ...
      {subview === 'add' ? (
        <>
          ...
          <button onClick={handleSave} disabled={!form.name.trim() || saving} className={`w-full py-3.5 rounded-xl text-[13px] font-semibold ${form.name.trim() ? 'bg-[#6B21A8] text-black' : 'bg-[#100C18] text-[#4B5563] border border-white/[0.06]'}`}>
            {saving ? 'Saving...' : editing ? 'Update Strategy' : 'Save Strategy'}
          </button>
          {editing && <button onClick={handleDelete} className="w-full py-3 rounded-xl text-[13px] font-medium text-[#EF4444] flex items-center justify-center gap-1.5"><Trash2 size={14} /> Delete Strategy</button>}
        </>
      ) : loading ? (
        <p className="text-[13px] text-[#6B7280] text-center py-8">Loading...</p>
      ) : strategies.length === 0 ? (
        <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-6 text-center">
          <p className="text-[13px] font-medium mb-1">No strategies saved yet</p>
        ...
        <div className="space-y-2">
          {strategies.map(s => {
            ...
            return (
              <button key={s.id} onClick={() => startEdit(s)} className="w-full text-left rounded-2xl bg-[#0B0810] border border-white/[0.06] p-4 active:bg-[#100C18]">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-[13px] font-semibold">{s.name}</p>
                  <p className="text-[13px] font-semibold" style={{ color: st.pnl > 0 ? '#22C55E' : st.pnl < 0 ? '#EF4444' : '#E8E9EC' }}>{st.pnl > 0 ? '+' : ''}{st.pnl.toFixed(2)}</p>
                </div>
  ...
  return (
    <>
      ...
      {(worstEmotion || costliestMistake) && (
        <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-4 space-y-2">
          <p className="text-[10px] uppercase tracking-wide text-[#6B7280] mb-1">Quick Insights</p>
          {worstEmotion && worstEmotion.pnl < 0 && (
            <p className="text-[13px] text-[#E8E9EC]">You lose the most when trading <span className="font-semibold text-[#EF4444]">{worstEmotion.key}</span> — {fmtMoney(worstEmotion.pnl)} across {worstEmotion.count} trade{worstEmotion.count !== 1 ? 's' : ''}.</p>
          )}
          {costliestMistake && costliestMistake.pnl < 0 && (
            <p className="text-[13px] text-[#E8E9EC]">Your costliest mistake is <span className="font-semibold text-[#EF4444]">{costliestMistake.key}</span> — {fmtMoney(costliestMistake.pnl)} total.</p>
          )}
          {!(worstEmotion && worstEmotion.pnl < 0) && !(costliestMistake && costliestMistake.pnl < 0) && (
            <p className="text-[13px] text-[#6B7280]">No clear costly patterns yet — keep logging to build a bigger picture.</p>
          )}
        </div>
      )}
      ...
      <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-4">
        ...
        {loading ? (
          ...
          <p className="text-[13px] text-[#6B7280]">No journal entries yet — log a daily entry in the Journal tab to track mood over time.</p>
        ) : (
          <div className="space-y-2">
            {journalEntries.slice(0, 8).map(e => (
              <div key={e.id} className="flex items-center justify-between">
                <p className="text-[13px]">{e.date}</p>
...
function EvidenceBlock({ title, children }) {
  return (
    <div>
      <p className="text-[9px] uppercase tracking-wide text-[#6B7280] mb-1">{title}</p>
      <div className="text-[13px] text-[#E8E9EC] leading-relaxed">{children}</div>
    </div>
  );
}
...
function WhyLosingView({ trades }) {
  ...
  if (!top) {
    return <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-5"><p className="text-[13px] text-[#6B7280]">No clear, data-backed cause stands out yet. That's a good sign — or it just means we need more trades to be sure.</p></div>;
  }
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-5 space-y-4">
        <div>
          <p className="text-[9px] uppercase tracking-wide text-[#EF4444] mb-1">Your Biggest Performance Leak</p>
          <p className="text-[15px] font-semibold">{top.name}</p>
        </div>
          ...
          <div className="space-y-2">
            {factors.slice(1).map((f, i) => (
              <div key={i} className="flex items-center justify-between">
                ...
                <p className="text-[13px] font-medium text-[#EF4444]">{fmtMoney(f.pnl)}</p>
              </div>
            ))}
          </div>
        </div>
...
function ImprovingView({ trades }) {
  ...
  if (n < 5) {
    return <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-5"><p className="text-[13px] text-[#6B7280]">Not enough trades yet for a fair before/after comparison — log more and check back.</p></div>;
  }
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-5">
        <p className="text-[9px] uppercase tracking-wide text-[#6B7280] mb-3">Last {n} Trades vs Previous {n}</p>
      ...
      <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-5">
        <p className="text-[9px] uppercase tracking-wide text-[#6B7280] mb-1">Coach's Verdict</p>
...
function TopMistakesView({ trades }) {
  ...
  if (mistakeTrades.length === 0) {
    return <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-5"><p className="text-[13px] text-[#6B7280]">No mistakes flagged in your trades yet — either great discipline, or the Mistake field isn't being used consistently.</p></div>;
  }
  ...
  return (
    <div className="space-y-2">
      {list.map((m, i) => {
        ...
        return (
          <div key={m.name} className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-4">
            <div className="flex items-center justify-between mb-1.5">
              <p className="text-[13px] font-semibold">#{i + 1} {m.name.toUpperCase()}</p>
              <span className="text-[9px] font-bold px-2 py-0.5 rounded" style={{ color: sev.color, backgroundColor: sev.color + '22' }}>{sev.label}</span>
            </div>
...
function EdgeLeakView({ trades }) {
  ...
  if (!edge) return <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-5"><p className="text-[13px] text-[#6B7280]">Not enough repeated combinations yet — log more trades with consistent strategy names to surface your edge.</p></div>;
  ...
  const Card = ({ title, g, positive }) => {
    ...
    return (
      <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-5">
        <p className="text-[9px] uppercase tracking-wide mb-1" style={{ color: positive ? '#22C55E' : '#EF4444' }}>{title}</p>
        <p className="text-[13px] font-semibold mb-3">{g.key}</p>
        <div className="grid grid-cols-3 gap-2">
          <div><p className="text-[9px] text-[#6B7280]">Trades</p><p className="text-[13px] font-semibold">{g.count}</p></div>
          <div><p className="text-[9px] text-[#6B7280]">Win Rate</p><p className="text-[13px] font-semibold">{((g.wins / g.count) * 100).toFixed(0)}%</p></div>
          <div><p className="text-[9px] text-[#6B7280]">Avg R</p><p className="text-[13px] font-semibold">{(g.rSum / g.count).toFixed(2)}R</p></div>
        </div>
        <p className="text-[13px] font-semibold mt-3" style={{ color: positive ? '#22C55E' : '#EF4444' }}>{fmtMoney(g.pnl)} net</p>
...
function renderFormatted(text) {
  return text.split('\n').map((line, li) => {
    ...
    if (headingMatch) {
      ...
      const sizeCls = level === 1 ? 'text-[13px] font-bold' : level === 2 ? 'text-[15px] font-bold' : 'text-[13px] font-semibold';
...
function AIMentorView({ trades, onClose }) {
  ...
  return (
    <>
      <button onClick={onClose} className="flex items-center gap-1 text-[13px] text-[#6B7280]"><ChevronLeft size={16} /> Back to Stats</button>
      <div className="rounded-2xl bg-gradient-to-br from-[#0B0810] to-[#08060D] border border-white/[0.06] p-5">
        <p className="text-[13px] font-semibold mb-1">RTrade AI Mentor</p>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {MENTOR_TOPICS.map(t => (
          <button key={t} onClick={() => setTopic(t)} className={`shrink-0 px-3.5 py-1.5 rounded-lg text-[11px] font-medium border ${topic === t ? 'bg-[#6B21A8]/15 border-[#6B21A8]/50 text-[#6B21A8]' : 'bg-[#100C18] border-white/[0.08] text-[#6B7280]'}`}>{t}</button>
        ))}
      </div>
      ...
      <div className="space-y-3">
        {messages.length === 0 && (
          <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-4">
            <p className="text-[13px] text-[#6B7280]">Ask something specific — e.g. "Why do I keep losing on short trades?" or "Is my risk sizing consistent?" The Mentor will use your real {topic.toLowerCase()} data to answer.</p>
          </div>
        )}
        {messages.map((m, i) => (
          <div key={i} className={`rounded-2xl p-4 max-w-[88%] ${m.role === 'user' ? 'bg-[#6B21A8]/15 ml-auto' : 'bg-[#0B0810] border border-white/[0.06]'}`}>
            <p className="text-[13px] leading-relaxed">{renderFormatted(m.content)}</p>
          </div>
        ))}
        {sending && <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-4 max-w-[88%]"><p className="text-[13px] text-[#6B7280]">Thinking...</p></div>}
      </div>
      ...
      <div className="flex gap-2 sticky bottom-2">
        <input
          ...
          className={inputCls}
        />
        <button onClick={sendMessage} disabled={sending || !input.trim()} className="px-4 rounded-xl bg-[#6B21A8] text-black text-[13px] font-semibold shrink-0 disabled:opacity-40">Send</button>
      </div>
    </>
  );
}
    ...
    <>
      ...
      {complianceRate !== null && (
        <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-5">
          ...
          <p className="text-[24px] font-semibold" style={{ color: complianceRate >= 80 ? '#22C55E' : complianceRate >= 60 ? '#F59E0B' : '#EF4444' }}>{complianceRate}%</p>
          <p className="text-[12px] text-[#6B7280] mt-1">{violatingTradeIds.size} of {trades.length} trades involved a rule violation</p>
        </div>
      )}

      <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-5 space-y-3">
        <p className="text-[10px] uppercase tracking-wide text-[#6B7280]">Your Rules</p>
        {loading ? <p className="text-[13px] text-[#6B7280]">Loading...</p> : rules.length === 0 ? (
          <p className="text-[13px] text-[#6B7280]">No rules set yet — add one below.</p>
        ) : (
          rules.map(r => (
            <div key={r.id} className="flex items-center justify-between rounded-xl bg-[#100C18] border border-white/[0.06] px-3.5 py-3">
              <div>
                <p className="text-[13px] font-medium">{RULE_TYPES[r.ruleType].label}</p>
        ...
        {availableTypes.length > 0 && (
          <div className="pt-2 border-t border-white/[0.06] space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleAddRule} disabled={!newThreshold || adding} className="px-4 rounded-xl bg-[#6B21A8] text-black text-[13px] font-semibold shrink-0 disabled:opacity-40">Add</button>
            </div>
          </div>
        )}
      </div>
    ...
    <>
      ...
      <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-5">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={saveBalance} disabled={saving || loading} className="px-4 rounded-xl bg-[#6B21A8] text-black text-[13px] font-semibold shrink-0 disabled:opacity-40">{savedMsg ? <Check size={16} /> : saving ? '...' : 'Save'}</button>
        </div>
      ...
      {avgRiskPct !== null && (
        <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-4">
          ...
          <p className="text-[15px] font-semibold">{avgRiskPct.toFixed(2)}% of starting balance</p>
      ...
      <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-5 space-y-4">
        ...
        {calcValid ? (
          <div className="grid grid-cols-2 gap-2 pt-1">
            <div className="rounded-xl bg-[#100C18] border border-white/[0.06] p-3 text-center"><p className="text-[9px] uppercase text-[#6B7280] mb-1">$ at Risk</p><p className="text-[13px] font-semibold">${calcRiskAmount.toFixed(2)}</p></div>
            <div className="rounded-xl bg-[#100C18] border border-white/[0.06] p-3 text-center"><p className="text-[9px] uppercase text-[#6B7280] mb-1">Position Size</p><p className="text-[13px] font-semibold">{calcPositionSize.toFixed(4)}</p></div>
          </div>
        ) : (
...
function CoachView({ trades, onClose }) {
  ...
  if (mode !== 'overview') {
    ...
    return (
      <>
        <button onClick={() => setMode('overview')} className="flex items-center gap-1 text-[13px] text-[#6B7280]"><ChevronLeft size={16} /> Back to Coach</button>
    ...
    <>
      ...
      <div className="rounded-2xl bg-gradient-to-br from-[#0B0810] to-[#08060D] border border-white/[0.06] p-5">
        <div className="flex items-center gap-2 mb-1">
          ...
          <p className="text-[13px] font-semibold">Trading Coach</p>
        </div>
      ...
      {trades.length < 5 ? (
        <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-6 text-center">
          <p className="text-[13px] font-medium mb-1">Not enough data yet</p>
        ...
        <>
          <div className="grid grid-cols-2 gap-2">
            {modes.map(m => (
              <button key={m.id} onClick={() => setMode(m.id)} className="py-3.5 rounded-xl text-[13px] font-medium bg-[#0B0810] border border-white/[0.06] text-[#E8E9EC] text-left px-3.5">{m.label}</button>
            ))}
          </div>
          ...
          {insights.length > 0 && (
            <div className="space-y-2">
              ...
              {insights.map((ins, i) => {
                const Icon = ins.icon;
                return (
                  <div key={i} className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-4 flex items-start gap-3">
                    ...
                    <p className="text-[13px] text-[#E8E9EC] leading-relaxed">{ins.text}</p>
                  </div>
                );
              })}
            </div>
...
function StatsView({ trades }) {
  ...
  if (trades.length === 0) {
    return (
      <>
        <div className="grid grid-cols-2 gap-2">
          <button onClick={() => setShowManager(true)} className="py-3 rounded-xl text-[13px] font-medium bg-[#0B0810] border border-white/[0.06] text-[#6B21A8] flex flex-col items-center justify-center gap-1"><Target size={14} /> Strategies</button>
          <button onClick={() => setShowPsych(true)} className="py-3 rounded-xl text-[13px] font-medium bg-[#0B0810] border border-white/[0.06] text-[#6B21A8] flex flex-col items-center justify-center gap-1"><Activity size={14} /> Psychology</button>
          <button onClick={() => setShowCoach(true)} className="py-3 rounded-xl text-[13px] font-medium bg-[#0B0810] border border-white/[0.06] text-[#6B21A8] flex flex-col items-center justify-center gap-1"><Flame size={14} /> Coach</button>
          <button onClick={() => setShowMentor(true)} className="py-3 rounded-xl text-[13px] font-medium bg-[#0B0810] border border-white/[0.06] text-[#6B21A8] flex flex-col items-center justify-center gap-1"><BookOpen size={14} /> AI Mentor</button>
          <button onClick={() => setShowRules(true)} className="py-3 rounded-xl text-[13px] font-medium bg-[#0B0810] border border-white/[0.06] text-[#6B21A8] flex flex-col items-center justify-center gap-1"><ShieldCheck size={14} /> Rule Engine</button>
          <button onClick={() => setShowRisk(true)} className="py-3 rounded-xl text-[13px] font-medium bg-[#0B0810] border border-white/[0.06] text-[#6B21A8] flex flex-col items-center justify-center gap-1"><TrendingDown size={14} /> Risk Mgmt</button>
        </div>
        <div className="rounded-2xl bg-[#0B0810] border border-white/[0.06] p-6 text-center">
          <p className="text-[13px] font-medium mb-1">No trades yet</p>
...
function JournalView() {
  ...
  return (
    <>
      ...
      <button onClick={handleSave} disabled={saving} className="w-full py-3.5 rounded-xl font-display text-[13px] font-semibold bg-[#6B21A8] text-black flex items-center justify-center gap-2">
      ...
      {!loading && entries.length > 0 && (
        <div className="pt-2">
          ...
          <div className="space-y-2">
            {entries.map(e => (
              <button key={e.id} onClick={() => loadEntry(e)} className="w-full text-left rounded-xl bg-[#0B0810] border border-white/[0.06] px-4 py-3 flex items-center justify-between active:bg-[#100C18]">
                <div><p className="text-[13px] font-medium">{e.date}</p><p className="text-[10px] text-[#6B7280]">{e.mood} · Confidence {e.confidence}/5</p></div>
...
function TradesTab({ trades, onAdd, onUpdate, onDelete, dbError }) {
  ...
  if (editingTrade) {
    return (
      <>
        <div className="flex items-center justify-between mb-1">
          <button onClick={() => setEditingTrade(null)} className="flex items-center gap-1 text-[13px] text-[#6B7280]"><ChevronLeft size={16} /> Back</button>
          <button onClick={async () => { if (window.confirm('Delete this trade permanently?')) { await onDelete(editingTrade.id); setEditingTrade(null); } }} className="flex items-center gap-1 text-[13px] text-[#EF4444]"><Trash2 size={14} /> Delete</button>
        </div>
  ...
  return (
    <>
      <div className="grid grid-cols-2 gap-2 mb-1">
        <button onClick={() => setSubview('add')} className={`py-2.5 rounded-xl text-[13px] font-medium border transition-colors ${subview === 'add' ? 'bg-[#6B21A8]/15 border-[#6B21A8]/50 text-[#6B21A8]' : 'bg-[#100C18] border-white/[0.08] text-[#6B7280]'}`}>Add Trade</button>
        <button onClick={() => setSubview('list')} className={`py-2.5 rounded-xl text-[13px] font-medium border transition-colors ${subview === 'list' ? 'bg-[#6B21A8]/15 border-[#6B21A8]/50 text-[#6B21A8]' : 'bg-[#100C18] border-white/[0.08] text-[#6B7280]'}`}>All Trades ({trades.length})</button>
      </div>
    ...
    <div className="min-h-screen w-full bg-[#050409] text-[#E8E9EC] font-sans flex flex-col overflow-x-hidden">
      <header className="px-5 pt-6 pb-4 flex items-center justify-between">
        <div>
          ...
          <h1 className="font-display text-[17px] font-semibold tracking-tight mt-0.5">{title}</h1>
        </div>
        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#2C0553] to-[#050409] border border-[#6B21A8]/30 flex items-center justify-center">
          ...
          {NAV_ITEMS.map(({ id, label, icon: Icon }) => {
            ...
            return (
              <button key={id} onClick={() => setActive(id)} className="flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-colors">
                ...
                <span className={`text-[9px] font-medium ${isActive ? 'text-[#6B21A8]' : 'text-[#6B7280]'}`}>{label}</span>
              </button>
            );
          })}
        </div>
